...
namespace Connector.App.v1.CompCode.Create;

public class CreateCompCodeHandler : IActionHandler<CreateCompCodeAction>
{
    private readonly ApiClient _apiClient;
    private readonly ConnectorRegistrationConfig _connectorRegistrationConfig;
    private readonly ILogger<CreateCompCodeHandler> _logger;

    public CreateCompCodeHandler(
        ApiClient apiClient,
        ConnectorRegistrationConfig connectorRegistrationConfig,
        ILogger<CreateCompCodeHandler> logger)
    {
    ...
    {
        var input = JsonSerializer.Deserialize<CreateCompCodeActionInput>(actionInstance.InputJson);
        if (input == null)
        {
            return ActionHandlerOutcome.Failed(new StandardActionFailure
            {
                Code = "400",
                Errors = [new Error { Source = ["CreateCompCodeHandler"], Text = "Invalid input" }] 
            });
        }
        ...
        try
        {
            ...
            var response = await _apiClient.CreateCompCodeDataObject(
                $"api/v1/companies/{_connectorRegistrationConfig.CompanyId}/comp_codes",
                input,
                cancellationToken)
            ...
            if (!response.IsSuccessful || response.Data == null)
            {
                return ActionHandlerOutcome.Failed(new StandardActionFailure
                {
                    ...
                    Errors = [
                        new Error
                        {
                            Source = [nameof(CreateCompCodeHandler)],
        ...
        catch (HttpRequestException exception)
        {
            ...
            var errorSource = new List<string> { nameof(CreateCompCodeHandler) };
