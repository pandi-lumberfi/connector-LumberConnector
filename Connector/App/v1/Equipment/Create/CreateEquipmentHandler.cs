...
namespace Connector.App.v1.Equipment.Create;
...
public class CreateEquipmentHandler : IActionHandler<CreateEquipmentAction>
{
    ...
    public async Task<ActionHandlerOutcome> HandleQueuedActionAsync(ActionInstance actionInstance, CancellationToken cancellationToken)
    {
        ...
        try
        {
            var relativeUrl = $"api/v1/companies/{_connectorRegistrationConfig.CompanyId}/equipments";
            // Given the input for the action, make a call to your API/system
            var response = new ApiResponse<CreateEquipmentActionOutput>();
            response = await _apiClient.CreateEquipment(relativeUrl, input, cancellationToken)
            ...
            if (!response.IsSuccessful || response.Data == null)
            {
                return ActionHandlerOutcome.Failed(new StandardActionFailure
                {
                    ...
                    Errors = [
                        new Error
                        {
                            Source = [nameof(CreateEquipmentHandler)],
