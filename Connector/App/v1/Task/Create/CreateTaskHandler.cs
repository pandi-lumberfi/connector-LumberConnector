...
namespace Connector.App.v1.Task.Create;

public class CreateTaskHandler : IActionHandler<CreateTaskAction>
{
    ...
    {
        var input = JsonSerializer.Deserialize<CreateTaskActionInput>(actionInstance.InputJson);
        try
        {
            if (input == null)
            {
            ...
            var relativeUrl = $"api/v1/companies/{_connectorRegistrationConfig.CompanyId}/projects/{input.ProjectId}/tasks";
            var response = new ApiResponse<CreateTaskActionOutput>();
            response = await _apiClient.CreateTask(relativeUrl, input, cancellationToken)
            ...
            if (!response.IsSuccessful || response.Data == null)
                return ActionHandlerOutcome.Failed(new StandardActionFailure
                {
                    ...
                    Errors = new []
                    {
                        new Error
                        {
                            Source = new [] { nameof(CreateTaskHandler) },
        ...
        catch (HttpRequestException exception)
        {
            ...
            var errorSource = new List<string> { nameof(CreateTaskHandler) };
